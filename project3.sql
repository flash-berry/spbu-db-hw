...
-- В текущей версии (задании) проекта были разработаны временные структуры и представления + способ валидации запросов.
...
-- Триггер, запрещающий назначать в поле manager_id работников, которые не являются менеджерами
CREATE OR REPLACE FUNCTION is_manager_check()
RETURNS TRIGGER AS $$
BEGIN
	PERFORM 1 FROM employees 
    WHERE employee_id = NEW.manager_id AND is_manager = TRUE;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'Работник с ID % не является менеджером. Назначение не разрешено.', NEW.manager_id;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER is_manager_check_trigger
BEFORE INSERT OR UPDATE 
ON employees
FOR EACH ROW
WHEN (NEW.manager_id IS NOT NULL)
EXECUTE FUNCTION is_manager_check();

INSERT INTO employees (first_name, last_name, position, is_manager, department_id, hire_date, salary, manager_id) VALUES
('Artem', 'Pyshnii', 'Devops', FALSE, 1, '2020-04-16', 87000.00, 2); -- вызовется is_manager_check_trigger и будет ошибка

INSERT INTO employees (first_name, last_name, position, is_manager, department_id, hire_date, salary, manager_id) VALUES
('Artem', 'Pyshnii', 'Devops', FALSE, 1, '2020-04-16', 87000.00, 1); -- вызовется is_manager_check_trigger и проверит что ошибки нет

UPDATE employees -- is_manager_check_trigger не вызовется
SET manager_id = NULL
WHERE employee_id = 6; 
...
UPDATE employees -- вызовется is_manager_check_trigger и будет ошибка
SET manager_id = 2
WHERE employee_id = 6; 

UPDATE employees -- вызовется is_manager_check_trigger и проверит что ошибки нет
SET manager_id = 5
WHERE employee_id = 6; 
...
-- Триггер на минимальную заработную плату
CREATE OR REPLACE FUNCTION check_salary()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.salary < 25000 THEN
        RAISE EXCEPTION 'Зарплата должна быть больше минимальной: 25000';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
UPDATE employees -- вызовется salary_check_trigger и будет ошибка
SET salary = 24999.00
WHERE employee_id = 6; 

UPDATE employees -- вызовется salary_check_trigger и проверит что ошибки нет
SET salary = 90000.00
WHERE employee_id = 6; 

INSERT INTO employees (first_name, last_name, position, is_manager, department_id, hire_date, salary, manager_id) VALUES
('Artem', 'Pyshnii', 'Devops', FALSE, 1, '2020-04-16', 24999.00, 1); -- вызовется salary_check_trigger и будет ошибка

INSERT INTO employees (first_name, last_name, position, is_manager, department_id, hire_date, salary, manager_id) VALUES
('Artem', 'Pyshnii', 'Devops', FALSE, 1, '2020-04-16', 35500.00, 1); -- вызовется salary_check_trigger и проверит что ошибки нет

-- Создадим вспомогательную таблицу для хранения логов по таблице employees
...
-- Триггер для логирования действий в таблице employees
CREATE OR REPLACE FUNCTION log_changes()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO employees_log (table_name, operation, changed_time)
...
INSERT INTO employees (first_name, last_name, position, is_manager, department_id, hire_date, salary, manager_id) VALUES
('Jasur', 'Barakaev', 'Devops', FALSE, 1, '2020-04-16', 45500.00, 1);

DELETE FROM employees
WHERE first_name = 'Jasur' AND last_name = 'Barakaev';

SELECT * FROM employees_log LIMIT 10;

-- Триггер, контролирует чтобы end_date не могла быть меньше start_date таблицы projects
CREATE OR REPLACE FUNCTION check_project_dates()
RETURNS TRIGGER AS $$
BEGIN
    IF (NEW.end_date IS NOT NULL AND NEW.start_date > NEW.end_date) THEN
        RAISE EXCEPTION 'Start_date не может быть позже, чем end_date';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
VALUES ('X', '2024-01-15', '2024-01-10', 50000.00); -- вызовется check_project_dates_trigger и будет ошибка

INSERT INTO projects (project_name, start_date, end_date, budget)
VALUES ('X', '2023-01-15', '2024-01-10', 50000.00); -- вызовется check_project_dates_trigger и проверит что ошибки нет

-- Создадим вспомогательную таблицу для архива сотрудников
...
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO employees_archive (employee_id, first_name, last_name, position, is_manager, department_id, hire_date, salary, manager_id)
    VALUES (OLD.employee_id, OLD.first_name, OLD.last_name, OLD.position, OLD.is_manager, OLD.department_id, OLD.hire_date, OLD.salary, OLD.manager_id);
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;
...
DELETE FROM employees -- -- вызовется archive_employee_trigger и after_changes_trigger
WHERE first_name = 'Artem' AND last_name = 'Pyshnii';

SELECT * FROM employees LIMIT 30;

SELECT * FROM employees_archive LIMIT 10;

SELECT * FROM employees_log LIMIT 10;

-- Транзакция для назначения нового менеджера для отдела
-- Успешная транзакция
BEGIN;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM employees WHERE employee_id = 5 AND is_manager = TRUE) THEN
        RAISE EXCEPTION 'Сотрудник не является менеджером';
    END IF;
END;
$$;
...
-- Транзакция для удаления сотрудника с проверкой зависимостей
-- Фейл транзакция
BEGIN;

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM employees WHERE manager_id = 5) THEN
        RAISE EXCEPTION 'Сотрудник назначен как менеджер для других сотрудников';
    END IF;
END;
$$;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM tasks WHERE assigned_to = 5) THEN
        RAISE EXCEPTION 'Сотрудник назначен на задачи';
    END IF;
END;
$$;
...
-- Фейл произошёл, потому что сотрудник является как минимум менеджером отдела

-- В текущей версии (задании) проекта были разработаны триггеры и транзакции.
-- Триггеры: is_manager_check_trigger, salary_check_trigger, after_changes_trigger, check_project_dates_trigger, archive_employee_trigger
-- И две транзакции: одна успешная другая фейл транзакция.